                ...
                new LoadCallBack<String>(context,false) {
                    ...
                    protected void onSuccess(Call call, Response response, String s) throws IOException {
                        if (s.contains("\"response\": \"ok\"")) {
                            LogUtils.showLog(ContentUrl.getUsersAssayList+"----患者检验列表同步数据", s);
                            AssayBeanOpe.deleteAllData(context);
                            Gson gson = new Gson();
                            AssayListBean usersCheckListBean = gson.fromJson(s, AssayListBean.class);
                            List<AssayBean> userCheckBeans = usersCheckListBean.getData();
                            AssayBeanOpe.insertData(context, userCheckBeans);
                        }else{
                            handler.removeMessages(DOINTERNET);
                        }
                    }
